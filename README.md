...
Node.js LTS (v18 or later recommended)
...
bash
npm install
4. Build the Smogon calc bundle
...
On any other device connected to the same network, open a browser and go to http://YOUR_IP:3000


Running as an always-on service (optional)
If you want the calculator to survive reboots without manually restarting it, use pm2:
bash
npm install -g pm2
...
For Windows users, you will need to use Task Scheduler
Open Powershell with Run as Administrator option

$action = New-ScheduledTaskAction -Execute "pm2" -Argument "resurrect" -WorkingDirectory "C:\pokedmgcalc\pokedmgcalc"
$trigger = New-ScheduledTaskTrigger -AtLogOn
$settings = New-ScheduledTaskSettingsSet -ExecutionTimeLimit 0
Register-ScheduledTask -TaskName "PM2 Startup" -Action $action -Trigger $trigger -Settings $settings -RunLevel Highest -Force

The one caveat is AtLogOn — it only fires when a user logs in, not on raw boot. If the OptiPlex reboots without anyone logging in, the server won't start. To fix that, change the trigger to run whether or not a user is logged in:
...
Data and privacy
Move lists, base stats, and learn methods are fetched live from PokéAPI — no API key required. All team and opponent data is stored locally in the SQLite database on your own machine. Nothing is sent to any external server.

Credits
Damage calculations powered by @smogon/calc — the same engine used by Pokémon Showdown.
