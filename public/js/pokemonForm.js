...
class PokemonForm {
  ...
  setState(s) {
    ...
    // Normalise moves to slugs regardless of how they were stored
  ...
  _renderStats() {
    ...
    const rows = STAT_NAMES.map((name, i) => {
      ...
      return `
        <div class="stat-row" style="display:grid;grid-template-columns:40px 58px 58px 1fr 58px 58px;gap:4px;align-items:center">
    ...
    return `
      <div class="form-row" style="gap:6px">
        <div style="display:grid;grid-template-columns:40px 58px 58px 1fr 58px 58px;gap:4px;margin-bottom:3px;">
  ...
  _renderMoves() {
    const moves = this.state.moves.map((mv, i) => {
      const mvSlug = mv ? mv.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/-$/, '') : '';
      const opts = this.availableMoves.map(m =>
        `<option value="${m}" ${m===mvSlug?'selected':''}>${_moveName(m)}</option>`
      ).join('');
      return `
  ...
  _bindEvents() {
    ...
    this.container.querySelectorAll('.move-sel').forEach(sel => {
      sel.addEventListener('change', () => {
        ...
        if (window.appState && window.appState.activeMoveSlot === idx &&
          ...
          window.appState.activeMoveSlug = sel.value;
          window.appCalc?.scheduleCalc();
        }
      });
    });

    // "Use" move buttons — sets which pokemon is attacking and which is defending
    this.container.querySelectorAll('.use-move-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // The pokemon that pressed "Use" is the attacker.
        // The other panel is the defender.
        window.appState.activeMoveSlot = idx;
        window.appState.activeMoveRole = this.role;
        window.appState.activeMoveSlug = moveSlug;
